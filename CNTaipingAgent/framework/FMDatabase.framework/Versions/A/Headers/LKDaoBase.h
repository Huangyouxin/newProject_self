...
typedef NS_ENUM(NSInteger, TransactionStyle) {
    TransactionStyleInsert,         
    TransactionStyleUpdate,
    TransactionStyleDelete,
    TransactionStyleClear
};
...
//数据库的 rowid
@property int rowid;
@property TransactionStyle transStyle;
//插入时是否忽略主键，一般用于把主键作为整形自增长
@property BOOL ignorePrimaryKey;
@end

@interface LKModelBase : JsonModelBase<LKModelBaseInteface>
@property int rowid;
@property TransactionStyle transStyle;
@property BOOL ignorePrimaryKey;
...
-(void)searchSQL:(NSString*)sql callback:(void(^)(NSArray*))block;
...
//把 model 插入到数据库，如果插入失败，可能是该主键数据已经存在，那么此时把新数据更新进去
-(void)insertOrUpdateToDB:(NSObject<LKModelBaseInteface>*)model callback:(void(^)(BOOL))block;
...
//通过事务进行操作, NSArray元素为(NSObject<LKModelBaseInteface>*)model
-(BOOL)transactionToDB:(NSArray*)models callback:(void(^)(BOOL))block;
