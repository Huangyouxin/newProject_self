...
/**
 *	@brief	如果本地对数据模型进行本地扩展，但不希望把这部分数据传给远程服务，请覆盖本函数
 *
 *	@return	忽略的属性
 */
...
//为了与数据库表模型相融合，特加入如下属性，这些属性不影响与服务器通信协议
//@{
@property int rowid;
@property int transStyle;
@property BOOL ignorePrimaryKey;
//}@
@end


@interface NSObject (Propertys)
+ (NSDictionary *)propertys;
+ (void) getPropertys:(NSMutableArray *)pronames protypes:(NSMutableArray *)protypes isGetSuper:(BOOL)isGetSuper;
@end